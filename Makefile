...
LEGACYFLAGS += -g -O0 -traceback -check all,noarg_temp_created -warn all -debug all -gen-interfaces -warn-interfaces
CFLAGS += -Wall
else
# optimised mode
# --------------
...
LEGACYFLAGS += $(IPO) -O3 -no-prec-div $(HOST) -w -vec-report0 -qopt-report0
endif
...
LEGACYFLAGS += -ffree-line-length-none -cpp -fno-stack-arrays 
MODFLAG = -J $(BUILD_DIR)
...
LEGACYFLAGS += -g -O0 -Wall -Wextra -fcheck=all -fbacktrace -ffpe-trap=zero,overflow 
CFLAGS += -Wall
else
# optimised mode
# --------------
...
SRCS := precision constants ode_solver opkda1 opkda2 opkdmain recfast
#SRCS := precision constants ode_solver dvode recfast
...
$(BUILD_DIR)/%.o: $(SOURCE_DIR)/%.f .base
	$(FC) $(FFLAGS) $(MODFLAG) -std=legacy -c $< -o $@
#$(FC) $(MODFLAG) -fPIC -cpp -fno-stack-arrays -g -O0 -fbacktrace -ffpe-trap=invalid,zero,overflow,underflow -std=legacy -c $< -o $@
#$(FC) $(LEGACYFLAGS) $(MODFLAG) -c $< -o $@
#LEGACYFLAGS = -fPIC -std=legacy
#LEGACYFLAGS += -ffree-line-length-none -cpp -fno-stack-arrays 
#LEGACYFLAGS += -g -O0 -Wall -Wextra -fcheck=all -fbacktrace -ffpe-trap=zero,overflow 
